...
public class NavigationCleanupWorker extends Worker {
  ...
  /**
   * Unique name for the periodic cleanup work so WorkManager keeps a single recurring chain instead
   * of stacking a new one each time navigation starts.
   */
  ...
  private void cleanDir() {
    ...
    if (BuildConfig.DEBUG) Log.d(TAG, "Time threshold - " + time.getTime().toString());
