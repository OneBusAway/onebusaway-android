 ...
 *
 * <p>This is a thin synchronous facade over the {@link PreferencesRepository} seam — every
 * read/write routes through the repository (resolved via {@link PreferencesEntryPoint}) rather than
 * touching {@code SharedPreferences} directly. That keeps these static helpers (and their many call
 * sites) working unchanged while the underlying store is free to move (e.g. to DataStore) behind
 * the seam.
 */
public class PreferenceUtils {
   ...
   *
   * <p>This is the one deliberately-kept {@code Application.get()} reach in this hub (#1636): it's
   * a pure DI graph handle — a {@link Context} used only to resolve the {@link
   * PreferencesEntryPoint}, never for its own state. The ~20 accessors here are static and reached
   * from ~90 call sites, most without a {@code Context} in scope, so threading one through purely
   * to resolve the same app-singleton would be churn with no behavioral gain. It can only be
   * removed by making {@code PreferenceUtils} itself injectable. The resource/string reaches that
   * <em>did</em> need a real {@code Context} (sort-order arrays, the location-denied key) now take
   * one as a parameter.
   */
   ...
   *
   * <p>Doubles are stored as the raw long bits of the value; passing the default's bits as the long
   * default makes an absent key round-trip back to {@code defaultValue} without a separate {@code
   * contains()} probe.
   *
  ...
  public static int getStopSortOrderFromPreferences(@NonNull Context context) {
    ...
    String sortPref =
        repo().getString(r.getString(R.string.preference_key_default_stop_sort), sortOptions[0]);
  ...
  public static int getReminderSortOrderFromPreferences(@NonNull Context context) {
    ...
    String sortPref =
        repo()
            .getString(
                resources.getString(R.string.preference_key_default_reminder_sort), sortOptions[0]);
    if (sortPref.equalsIgnoreCase(sortOptions[0])) {
      return 0;
    } else if (sortPref.equalsIgnoreCase(sortOptions[1])) {
      return 1;
  ...
  /**
   * Retrieves the map view location and zoom level from a preference and stores it in the provided
   * bundle, if a valid lat/long and zoom level has been previously saved to prefs
   *
  ...
  /**
   * Returns true if the user has previously indicated that they don't want to be prompted to
   * provide location permissions. Note that this means they haven't actually be prompted with the
   * system permission dialog.
   */
  public static boolean userDeniedLocationPermission(@NonNull Context context) {
    Resources r = context.getResources();
    return getBoolean(
        r.getString(R.string.preferences_key_user_denied_location_permissions), false);
  }

  /**
   * Set value to true if the user has previously indicated that they don't want to be prompted to
   * provide location permissions, or false if they have indicated that they want to be prompted
  ...
  /**
   * Returns true if preferred units are metric, false if Imperial. If set to Automatic, assume
   * Imperial if the default locale is the US, metric otherwise.
   *
  ...
  public static boolean getUnitsAreMetricFromPreferences(@NonNull Context context) {
    ...
    String preferredUnits =
        repo().getString(context.getString(R.string.preference_key_preferred_units), AUTOMATIC);
