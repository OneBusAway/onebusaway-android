 ...
 *  - `null` filter → show all places (returns [all])
 ...
 *  - non-empty filter → only the places whose id is in the filter
 */
internal fun filterRentals(
    all: List<RentalPlace>,
    selectedIds: List<String>?
): List<RentalPlace>? = when {
    selectedIds == null -> all
    selectedIds.isEmpty() -> null
    else -> all.filter { selectedIds.contains(it.id) }
}

/** What the rental loader should do with the overlay for the current viewport. */
internal enum class RentalAction {
    /** Load rentals for the viewport and show the [filterRentals] result. */
    SHOW,

    /** Clear the rental overlay (both layers toggled off, and not directions-with-stations). */
    CLEAR,
 ...
 * since it reads `Application`). Rentals always show in directions mode once the itinerary's stations
 * are known; otherwise they follow the layer toggles — either of which showing keeps the layer alive.
 *
 * @param isDirections whether the map is in directions mode
 * @param selectedIds the itinerary's rental filter: null = not a directions itinerary (show all per
 * the toggles), empty = a directions itinerary with no rentals, non-empty = those ids
 * @param visibleLayers the bikes/scooters layer toggles that are currently on
 */
internal fun rentalAction(
    isDirections: Boolean,
    selectedIds: List<String>?,
    visibleLayers: Set<RentalLayer>
): RentalAction {
    val show = if (isDirections && !selectedIds.isNullOrEmpty()) true else visibleLayers.isNotEmpty()
    return when {
        !show -> RentalAction.CLEAR
        isDirections && selectedIds == null -> RentalAction.LEAVE
        else -> RentalAction.SHOW
    }
}
