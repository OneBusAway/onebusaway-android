...
public class NavigationServiceProvider implements TextToSpeech.OnInitListener {
  ...
  // Index that defines the current path link within the path (i.e. First link in a path will have
  // index = 0, second link index = 1, etc.)
  ...
  @Nullable public static TextToSpeech mTTS; // TextToSpeech for speaking commands.

  private String mTripId; // Trip ID
  private String mStopId; // Stop ID

  // Application context, threaded from the hosting NavigationService (replaces the Application
  // static).
  private final Context mContext;

  public NavigationServiceProvider(
      @NonNull Context context, @Nullable String tripId, @Nullable String stopId) {
  ...
  public NavigationServiceProvider(
      @NonNull Context context, @Nullable String tripId, @Nullable String stopId, int flag) {
    Log.d(TAG, "Creating NavigationServiceProvider...");
    mContext = context.getApplicationContext();
    mResuming = flag == 1;
    if (mTTS == null) {
  ...
  /**
   * Initialize ProximityCalculator Proximity listener will be created only upon selection of
   * service to navigate
   */
  ...
  public void navigate(@NonNull Path path) {
    ...
    Location secondToLastLocation =
        mPath.getPathLinks().get(mPathLinkIndex).getSecondToLastLocation();
  ...
  /**
   * Returns true if there is another path link to be navigated as part of the current path that is
   * being navigated, false if there is not another link
   *
   * @return true if there is another path link to be navigated as part of the current path that is
   *     being navigated, false if there is not another link
   */
  ...
  private void navigateNextPathLink() {
    ...
    if (mPath != null && mPathLinkIndex < mPath.getPathLinks().size()) {
      ...
      mProxCalculator.listenForLocation(
          link.getOriginLocation(), link.getSecondToLastLocation(), link.getDestinationLocation());
      Log.d(TAG, "ProxCalculator parameters were set!");
    }
  }

  /**
   * Called from LocationListener.locationUpdated() in order to supply the Navigation Provider with
  ...
  public class ProximityCalculator {
    ...
    // Defines radius (in meters) for which the Proximity listener should be triggered (Default =
    // 50)
     ...
     *
     * @param eventType EVENT_TYPE_PULL_CORD for "Pull the cord now" event, or EVENT_TYPE_GET_READY
     *     for getting ready to exit the vehicle
     * @param alertState ALERT_STATE_* variable is responsible for differentiating the switch of
     *     path link and alert being played.
     */
    boolean proximityEvent(int eventType, int alertState) {
      // *******************************************************************************************************************
      // * This function is fired by the ProximityListener when it detects that it is near a set of
    ...
    /**
     * Fire proximity event to switch path link or go back to service menu when the final stop of
     * the path link or path is reached stop_type = 0; -> final stop detection stop_type = 1; ->
     * second to last stop detection speed = current speed of the bus;
     */
    ...
    /**
     * Checks the proximity to the provided location and returns the event type (EVENT_TYPE_*) that
     * was triggered by the proximity to this location.
     *
     * @return the status update (EVENT_TYPE_*) that was triggered by the proximity to this location
     *     (if any).
     */
    private int checkProximityAll(Location currentLocation) {
      if (!mWaitingForConfirm) {
        ...
        Log.d(
            TAG,
            "Second to last stop coordinates: "
                + secondToLastCoords.getLatitude()
                + ", "
                + secondToLastCoords.getLongitude());
        ...
        if (directDistance < DISTANCE_THRESHOLD) {
          if (proximityEvent(EVENT_TYPE_GET_READY, ALERT_STATE_NONE)) {
            ...
            Log.d(TAG, "-----Get ready!");
            AnalyticsEntryPoint.get(mContext)
                .reportUiEvent(
                    PlausibleAnalytics.REPORT_DESTINATION_REMINDER_EVENT_URL,
                    mContext.getString(R.string.analytics_label_destination_reminder),
                    mContext.getString(
                        R.string.analytics_label_destination_reminder_variant_get_ready));
          ...
          if (proximityEvent(EVENT_TYPE_PULL_CORD, ALERT_STATE_SHOWN_TO_RIDER)) {
            ...
            AnalyticsEntryPoint.get(mContext)
                .reportUiEvent(
                    ...
                    mContext.getString(R.string.analytics_label_destination_reminder),
                    mContext.getString(
                        R.string.analytics_label_destination_reminder_variant_exit_at_next_stop));
  ...
  /**
   * Gets the notification to be used for starting a hosting service for the
   * NavigationServiceProvider in the foreground
   *
   * @return the notification to be used for starting a hosting service for the
   *     NavigationServiceProvider in the foreground
   */
  @NonNull
  ...
  /**
   * Updates the user interface (e.g., distance display, speech) based on navigation events TODO -
   * This method should be moved to a NavigationServiceListener class based on a listener interface
   *
   * @param eventType EVENT_TYPE_* variable defining the eventType update to act upon
   * @return the notification to use for the foreground service if eventType ==
   *     EVENT_TYPE_INITIAL_STARTUP, otherwise returns null
   */
  private Notification updateUi(int eventType) {
    TripDetailsLauncher.Builder bldr = new TripDetailsLauncher.Builder(mContext, mTripId);
    ...
    PendingIntent pIntent = getActivity(mContext, 1, intent, flags);
    ...
    if (eventType == EVENT_TYPE_INITIAL_STARTUP) {
      ...
      PendingIntent pCancelIntent = getBroadcast(mContext, 0, receiverIntent, cancelFlags);
      mBuilder.addAction(
          R.drawable.ic_navigation_close,
          mContext.getString(R.string.destination_reminder_cancel_trip),
          pCancelIntent);
      ...
      String preferredUnits =
          PreferenceUtils.getString(
              mContext.getString(R.string.preference_key_preferred_units), AUTOMATIC);
      ...
      if (useImperial) {
        if (miles < 0.1) {
          ...
          mBuilder.setContentText(
              mContext
                  .getResources()
                  .getQuantityString(R.plurals.distance_feet, roundedFeet, roundedFeet));
        } else {
          // Show miles
          mBuilder.setContentText(
              mContext
                  .getResources()
                  .getQuantityString(R.plurals.distance_miles, (int) miles, fmt.format(miles)));
        }
      } else {
        if (kilometers < 1) {
          // Show meters when distance is less than 1 km
          mBuilder.setContentText(
              mContext.getResources().getQuantityString(R.plurals.distance_meters, meters, meters));
        } else {
          // Show kilometers
          mBuilder.setContentText(
              mContext
      ...
      receiverIntent.putExtra(
          NavigationReceiver.ACTION_NUM, NavigationReceiver.DISMISS_NOTIFICATION);
      PendingIntent pDelIntent = getBroadcast(mContext, 0, receiverIntent, cancelFlags);
