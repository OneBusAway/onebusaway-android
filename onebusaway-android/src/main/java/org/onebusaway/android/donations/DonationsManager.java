...
public class DonationsManager {
  ...
  // Application context, used at event time to (a) read Resources for event labels / the donate URL
  // and
  // (b) lazily resolve the AnalyticsProvider (for the region-derived Plausible emitter). Resolving
  // the
  // AnalyticsProvider lazily — rather than injecting it into this constructor — keeps the
  // constructor off
  // the region graph: AnalyticsProvider pulls in the RegionRepository, which is deliberately
  // constructed
  ...
  // The constructor only captures its collaborators — no dereferencing — so a bare instance can
  // back
  // the JVM unit tests of consumers whose exercised paths never touch the manager. Resources and
  // the
  // launch count are read lazily at their use sites.
  @Inject
  public DonationsManager(
      @ApplicationContext @Nullable Context context, @Nullable PreferencesRepository preferences) {
   ...
   * @param state the state or variant of the UI item, or null if the item doesn't have a state or
   *     variant
   */
  ...
  /**
   * @return The date that donation requests were hidden by the user, either because they donated or
   *     because they tapped the 'dismiss' button, or null if the date has not been set.
   */
  @Nullable
  ...
  public void setDonationRequestDismissedDate(@Nullable Date date) {
    PreferenceUtils.saveLong(donationRequestDismissedDateKey, date == null ? -1 : date.getTime());
  }
  ...
  public void setDonationRequestReminderDate(@Nullable Date date) {
    PreferenceUtils.saveLong(donationRequestReminderDateKey, date == null ? -1 : date.getTime());
  }

  /** Sets a date two weeks in the future on which the app will remind the user to donate. */
  public void remindUserLater() {
    long twoWeeksInMilliseconds =
        86400 * 14 * 1000; // Seconds in a day * 14 days * 1000 milliseconds
   ...
   * ViewModel (the settings / learn-more nav destinations). ViewModel-backed UI reports via {@link
   * #reportDonateButtonPress()} and lets its host build the intent from {@link #donateUrl()}.
   */
  @NonNull
