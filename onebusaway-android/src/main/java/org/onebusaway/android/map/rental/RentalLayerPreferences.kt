 ...
 * chrome (which tints the buttons), `LayerUtils` (the resume-time push), and `RentalLayerController`
 * (which recomputes after every tap).
 *
 ...
 * layer carries its on/off choice straight across onto the new master.
 */
fun rentalLayersFromPreferences(prefs: PreferencesRepository): Set<RentalLayer> {
    if (!prefs.getBoolean(R.string.preference_key_layer_bikeshare_visible, true)) return emptySet()
