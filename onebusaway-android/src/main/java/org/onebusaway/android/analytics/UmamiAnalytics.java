...
/**
 * Fire-and-forget Umami analytics emitter. POSTs events to {@code <serverUrl>/api/send} with a
 * device-like User-Agent. All failures are swallowed; this class never throws into its callers.
 * Disabled regions never construct an instance (see Application).
 */
public class UmamiAnalytics {
  ...
  public UmamiAnalytics(
      @Nullable String serverUrl, @Nullable String websiteId, @Nullable String hostname) {
  ...
  public void event(
      @Nullable String name, @Nullable String pageUrl, @Nullable Map<String, Object> props) {
  ...
  public @NonNull String buildPayload(
      @Nullable String name, @Nullable String path, @Nullable Map<String, Object> props)
  ...
  /**
   * Umami returns HTTP 200 even when it silently drops a request (bot-like User-Agent or bad
   * config), replying with {@code {"beep":"boop"}}. Treat that as a failure.
   */
