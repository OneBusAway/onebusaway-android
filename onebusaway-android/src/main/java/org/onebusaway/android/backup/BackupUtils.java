...
public class BackupUtils {
  ...
  public static void restore(
      @NonNull Context activityContext, @NonNull Uri uri, @Nullable Runnable onRestored) {
    //
   ...
   *     Android 11 we can't access this directory and need to rely on the system UI picker.
   */
  private static void doRestore(Context activityContext, Uri uri, Runnable onRestored) {
    ...
    try {
      ...
      Toast.makeText(
              context,
              context.getString(
                  R.string.preferences_db_restored, context.getString(R.string.app_name)),
              Toast.LENGTH_LONG)
          .show();
  ...
  /**
   * Builds the intent that lets the user select a backup file to restore. Launched via an {@code
   * ActivityResultLauncher}; the picked URI is passed back to {@link #restore(Context, Uri)}.
   */
  @NonNull
