...
/**
 * A Hilt [EntryPoint] that lets code which can't be constructor- or field-injected — `Application`'s
 * static location delegates, static Java utilities — reach the location seam. It needs a [Context]
 * only to resolve the singleton graph; the returned repository/sink is the same app-singleton every
 * injected consumer shares.
 *
 * Use it only where injection genuinely isn't available — Hilt-reachable classes (Activities,
 * Fragments, Services, ViewModels, Workers) should inject [LocationRepository] directly. The
 * write-side [LocationSink] is reached only by the device-listener ingestion path.
 */
@EntryPoint
