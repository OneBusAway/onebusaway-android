...
fun MyStopsDestination(
    ...
    val search = rememberSearchVm("stops.search") {
        DefaultStopSearchRepository(
            SearchCenter(LocationEntryPoint.get(app), RegionEntryPoint.get(app)),
            app,
            NetworkEntryPoint.getLocationSearch(app)
        )::search
    }
    PersistedTabsScreen(
